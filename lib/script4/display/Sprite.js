class Sprite extends Phaser.Group
{
	...
	touchEvent(object, pointer, isDown)
	{
		if (!(object instanceof ButtonSuper)) {
			...
			if (isDown == undefined) {
				...
				object.parent.touchEventCallBack(new TouchEvent(TouchPhase.BEGAN, object.parent, object));
			} else {
				object.parent.isTouchDown = false;
				object.game.input.deleteMoveCallback(object.parent.onMove, object);
				object.parent.touchEventCallBack(new TouchEvent(TouchPhase.ENDED, object.parent, object));
			}
		}
	}
	...
	onMove()
	{
		this.parent.touchEventCallBack(new TouchEvent(TouchPhase.MOVED, this.parent, this));
	}
