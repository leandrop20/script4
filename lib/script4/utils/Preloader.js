	...
	create() {
		for (var i=0;i<Assets.ASSETS.length;i++) {
			switch(Assets.ASSETS[i].type) {
				case 'image':
					this.load.image(Assets.ASSETS[i].name, Core.basePath + Assets.ASSETS[i].path);
					break;
				case 'audio':
					this.load.audio(Assets.ASSETS[i].name, Core.basePath + Assets.ASSETS[i].path);
					break;
				case 'spine':
					this.load.spine(Assets.ASSETS[i].name, Core.basePath + Assets.ASSETS[i].path);
					break;
				case 'spritesheet':
					this.load.spritesheet(Assets.ASSETS[i].name, Core.basePath + Assets.ASSETS[i].path);
					break;
				case 'atlas':
					this.load.atlas(Assets.ASSETS[i].name,
						Core.basePath + Assets.ASSETS[i].path.substr(0, Assets.ASSETS[i].path.length-4) + 'png',
						Core.basePath + Assets.ASSETS[i].path
					);
					break;
				case 'bitmapfont':
					this.load.bitmapFont(Assets.ASSETS[i].name, 
						Core.basePath + Assets.ASSETS[i].path.substr(0, Assets.ASSETS[i].path.length-3) + 'png',
						Core.basePath + Assets.ASSETS[i].path
					);
					 break;
				case 'particle':
					...
					this.load.json(Assets.ASSETS[i].name + 'Settings',
						Core.basePath + Assets.ASSETS[i].path.substr(0, Assets.ASSETS[i].path.length-3) + 'json',
						Core.basePath + Assets.ASSETS[i].path
					);
					break;
				case 'xml':
					this.load.xml(Assets.ASSETS[i].name, Core.basePath + Assets.ASSETS[i].path);
					break;
			}
		}
