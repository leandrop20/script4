		...
		for (var i=0;i<Assets.ASSETS.length;i++) {
			switch(Assets.ASSETS[i].type) {
				...
				case 'spritesheet':
					...
					break;
				case 'atlas':
					this.load.atlas(Assets.ASSETS[i].name, Assets.ASSETS[i].path.substr(0, Assets.ASSETS[i].path.length-4) + 'png', Assets.ASSETS[i].path);
					break;
				case 'bitmapfont':
					this.load.bitmapFont(Assets.ASSETS[i].name, Assets.ASSETS[i].path.substr(0, Assets.ASSETS[i].path.length-3) + 'png', Assets.ASSETS[i].path);
					 break;
				case 'particle':
					this.load.image(Assets.ASSETS[i].name, Assets.ASSETS[i].path);
					this.load.json(Assets.ASSETS[i].name + 'Settings', Assets.ASSETS[i].path.substr(0, Assets.ASSETS[i].path.length-3) + 'json', Assets.ASSETS[i].path);
					break;
			}
		}
