class TouchEvent
{
	...
	getTouch(target)
	{
		...
		if (!Touch.previous || Touch.previous == undefined) {
			...
			} else if (isIntersects(target) && (obj.inputEnabled || obj.inputEnableChildren)) {
		...
		{
			console.log(_target);
			// var bounds = _target.getBounds();
			// var boundsCenterLocalCoord = new Phaser.InputHandler(core).globalToLocal(new Phaser.Point(bounds.centerX, bounds.centerY));
			// globalX = globalX + (_target.x - bounds.centerX);
			// globalX = globalX + (_target.x - boundsCenterLocalCoord.x);
            // globalY = globalY + (_target.y - bounds.centerY);
