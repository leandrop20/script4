...
export class Scroller {
	...
	startScroll($point: Point) {
		...
		if (this._isScrollBegin) {
			...
			function initScrollV(_this: any) {
			...
			function initScrollH(_this: any) {
		...
		function scrollVSetting(_this: any) {
		...
		function scrollHSetting(_this: any) {
			//if maskContent's position exceeds the bounds, make it drag only half as far with each mouse movement (like iPhone/iPad behavior)
	...
	computeYPerc($manualPerc: boolean = false) {
	...
	computeXPerc($manualPerc: boolean = false) {
		if (!this._content) return;
