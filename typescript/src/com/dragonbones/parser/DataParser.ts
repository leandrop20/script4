...
export class DataParser {
    ...
    _animation: any;
    _timeline: any;
    _isOldData!: boolean; // For 2.x ~ 3.x
    _isGlobalTransform!: boolean; // For 2.x ~ 3.x
    _isAutoTween!: boolean; // For 2.x ~ 3.x
    _animationTweenEasing!: number; // For 2.x ~ 3.x
    _timelinePivot!: Point; // For 2.x ~ 3.x
    _helpPoint!: Point;
    _helpTransformA!: Transform;
    _helpTransformB!: Transform;
    _helpMatrix!: Matrix;
    _rawBones: any[]; // For skinned mesh
	...
	static _getArmatureType(value: any) {
    ...
    static _getDisplayType(value: any) {
        switch (value.toLowerCase()) {
            case "image":
                return 0 /* Image */;
            case "armature":
    ...
    static _getBlendMode(value: any) {
        switch (value.toLowerCase()) {
            case "normal":
                return 0 /* Normal */;
            case "add":
    ...
    static _getActionType(value: any) {
        switch (value.toLowerCase()) {
            case "play":
                return 0 /* Play */;
            case "stop":
    ...
    _getTimelineFrameMatrix(animation: any, timeline: any, position: any, transform: any) {
    ...
    _globalToLocal(armature: any) {
        ...
        for (var i = 0, l = bones.length; i < l; ++i) {
            ...
            for (var i_2 in armature.animations) {
                ...
                var parentTimeline = bone.parent
                    ? animation.getBoneTimeline(bone.parent.name)
                    : null;
     ...
     */
    static parseDragonBonesData(rawData: any): any {
     ...
     */
    static parseTextureAtlasData(rawData: any, scale: number = 1): any {
