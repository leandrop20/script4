...
export class DragonBones extends Sprite {
    ...
    static readonly PI_D: number = Math.PI * 2;
    static readonly PI_H: number = Math.PI / 2;
    static readonly PI_Q: number = Math.PI / 4;
    static readonly ANGLE_TO_RADIAN: number = Math.PI / 180;
    static readonly RADIAN_TO_ANGLE: number = 180 / Math.PI;
    static readonly SECOND_TO_MILLISECOND: number = 1000;
    static readonly NO_TWEEN: number = 100;
    static readonly VERSION: string = '4.7.2';

    private static _GAME: any;
    private static _debug: boolean;
    private static _debugDraw: boolean;
    private static __armatures: any[];

    static get GAME(): any { return DragonBones._GAME; }
	static set GAME(value: any) { DragonBones._GAME = value; }

    static get debug(): boolean { return (DragonBones._debug) ? DragonBones._debug : false; }
    static set debug(value: boolean) { DragonBones._debug = value; }
    
    static get debugDraw(): boolean {
        return (DragonBones._debugDraw) ? DragonBones._debugDraw : false;
    }
    static set debugDraw(value: boolean) { DragonBones._debugDraw = value; }
    
    static get _armatures(): any[] {
        return (DragonBones.__armatures) ? DragonBones.__armatures : [];
    }
    static set _armatures(value: any[]) { DragonBones.__armatures = value; }
...
/* PIXI.WebGLSpriteBatch = function (game: any) {
    this.game = game;
    this.vertSize = 5;
