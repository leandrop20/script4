...
import { TransformObject } from '../geom/TransformObject';
...
export class Slot extends TransformObject {
    ...
    _updateDisplay() {
        ...
        // Update origin.
        if (
            this._displayDataSet &&
            this._displayIndex >= 0 &&
            this._displayIndex < this._displayDataSet.displays.length
        ) {
            ...
            if (this._childArmature) {
                ...
                if (this.inheritAnimation) {
                    ...
                    var actions = slotData.actions.length > 0
                        ? slotData.actions
                        : this._childArmature.armatureData.actions;
    ...
    _updateMeshData(isTimelineUpdate: any) {
        ...
        if (this._display && this._display == this._meshDisplay && this._displayIndex >= 0) {
            ...
            var replaceDisplayData = (this._displayIndex < this._replacedDisplayDataSet.length)
        ...
        if (self._transformDirty) {
            ...
            if (self.globalTransformMatrix == self._globalTransformMatrix) {
                ...
                if (cacheFrameIndex >= 0 && !self._cacheFrames[cacheFrameIndex]) {
                    self.globalTransformMatrix = SlotTimelineData.cacheFrame(
                        self._cacheFrames,
                        cacheFrameIndex,
                        self._globalTransformMatrix
                    );
                }
            }
