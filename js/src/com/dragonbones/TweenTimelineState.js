...
export default class TweenTimelineState extends TimelineState {

	constructor() {
		super();
	}

	static _getEasingValue(progress, easing) {
        if (progress <= 0) {
            return 0;
        }
        else if (progress >= 1) {
    ...
    static _getCurveEasingValue(progress, sampling) {
        if (progress <= 0) {
            return 0;
        }
        else if (progress >= 1) {
    ...
    _updateExtensionKeyFrame(current, next, result) {
